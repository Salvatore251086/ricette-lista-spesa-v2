...
// Pulito, robusto, niente trick strani.
...
// Log avvisi se manca qualcosa, non blocca
{
...
function extractYoutubeId (str) {
  ...
  const m = String(str).match(/(?:v=|youtu\.be\/|embed\/)([A-Za-z0-9_-]{11})/)
  return m ? m[1] : null
}

function getVideoUrl (r) {
  // priorità: video.url, links.video, enrichedFrom.video, youtubeId, ytID, video.id, enrichedFrom.youtubeId
  const direct =
    r?.video?.url ||
    r?.links?.video ||
    r?.enrichedFrom?.video ||
    null
...
function openVideo (url) {
  ...
  dom.videoModal.removeAttribute('aria-hidden')
  dom.videoModal.focus()
}
...
function renderRecipes () {
  ...
  filtered.forEach(r => {
    ...
    const hasVid = Boolean(videoUrl)
    const videoLabel = hasVid ? 'Guarda video' : 'Video n/d'
    ...
    card.innerHTML = `
      ...
      <div class="recipe-actions">
        ...
        ${
          hasVid
            ? `<button class="btn video" data-video="${encodeURI(videoUrl)}">${videoLabel}</button>`
